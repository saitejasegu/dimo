...
/** Client island for WorkOS sign-in on the public homepage. */
export function SignInButtons() {
  ...
  const signIn = async (provider: AuthProvider) => {
    if (!window.isSecureContext || !window.crypto?.subtle) {
      setSignInError("Sign-in requires HTTPS.");
      return;
    }
    try {
      setReady(false);
  ...
  return (
    <div className={`space-y-3 ${ready ? "" : "pointer-events-none opacity-60"}`}>
      <Button
        fullWidth
        variant="contrast"
        leftIcon={<AppleMark />}
      ...
      {signInError ? <p className="text-center text-sm text-danger">{signInError}</p> : null}
