...
- Swift values sent to Convex `v.number()` fields must be encoded as `Double`;
  encoding Swift `Int` produces Convex `$integer` and fails validation.
...
- Native iOS is the only lending writer; web and Electron must remain
  read-only.
