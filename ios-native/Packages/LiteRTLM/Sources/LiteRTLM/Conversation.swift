...
public class Conversation {
  ...
  private let cancelNativeProcessing: (CConversationHandle) -> Void
  private let deleteNativeConversation: (CConversationHandle) -> Void

  /// Whether the conversation is alive and ready to be used.
  public var isAlive: Bool {
    handleLock.withLock { handle != nil }
  }

  convenience init(handle: CConversationHandle, toolManager: ToolManager) {
    self.init(
      handle: handle,
      toolManager: toolManager,
      cancelNativeProcessing: { litert_lm_conversation_cancel_process($0) },
      deleteNativeConversation: { litert_lm_conversation_delete($0) }
    )
  }
  ...
  init(
    ...
    toolManager: ToolManager,
    cancelNativeProcessing: @escaping (CConversationHandle) -> Void,
    deleteNativeConversation: @escaping (CConversationHandle) -> Void
  ) {
    self.handle = handle
    self.toolManager = toolManager
    self.cancelNativeProcessing = cancelNativeProcessing
    self.deleteNativeConversation = deleteNativeConversation
  }
  ...
  /// Deterministically releases the native session. LiteRT-LM supports one
  /// live session per engine, so relying on ARC timing can make the next
  /// conversation fail with `A session already exists`.
  public func close() {
    let ownedHandle = handleLock.withLock { () -> CConversationHandle? in
      let value = handle
      handle = nil
      return value
    }
    guard let ownedHandle else { return }
    cancelNativeProcessing(ownedHandle)
    deleteNativeConversation(ownedHandle)
  }
  ...
  fileprivate func checkIsAlive() throws -> OpaquePointer {
    guard let handle = handleLock.withLock({ handle }) else {
